...
void usage (char * prog) {
	int i;
	fprintf (stderr, "usage: %s seed count [ size_profile alloc_profile ]\n", prog);
...
int main (int argc, char * argv[])
{
	...
	for (i=0; i<count || nptr; ++i) {
		...
		if (i < count && (nptr == 0 || isalloc > 0)) {
			...
			my_assert (nptr < DATACOUNT, "Anzahl der Operationen zu gross");
			alloc += sz;
			nalloc++;
			if (nalloc > maxnalloc)
				maxnalloc = nalloc;
			...
			memcpy (data[nptr].ptr, data[nptr].contents, sz);
			{
				...
				ptr2 = ptr = (unsigned long)data[nptr].ptr;
				/* Speicher nicht auf einer 8 Byte Kante,
				 * oder nicht in einem Block, den
				 * get_block_from_system geliefert hat. */
				my_assert (valid_area (ptr, sz), "Speicher nicht auf einer 8 Byte Kante oder nicht in einem Block, den get_block_from_system geliefert hat");
				n = find_avl (areas, ptr);
				/* Ueberlappende Speicherbereiche. */
				my_assert (n->start + n->len <= ptr, "Ueberlappende Speicherbereiche");
				if (n->next) {
					ptr2 += sz;
					/* Ueberlappende Speicherbereiche. */
					my_assert (ptr2 <= n->next->start, "Ueberlappende Speicherbereiche");
				}
			...
			my_assert (ret == 0, "Allokierter Speicherbereich wurde zwischenzeitlich veraendert");
...
#endif
			{
				unsigned long ptr;
				struct avl_node * n;
				ptr = (unsigned long)data[idx].ptr;
				n = find_avl (areas, ptr);
				assert (n->start == ptr);
	...
	v1 = -1.0 + ((double) BLOCKSIZE * get_sys_blockcount ())/((double)(maxalloc+8*maxnalloc));
	v2 = (double)usecs/(double)i;
	printf ("Relative size overhead: %lf\n", -1.0 + ((double) BLOCKSIZE * get_sys_blockcount ())/((double)(maxalloc+8*maxnalloc)));
